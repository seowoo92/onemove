...
- **카드 레이아웃 (7/23 컴팩트 개편, 시안 B 확정)**: 칩 행(난이도·영역 + 우측 ★·새로고침 아이콘) → 루틴명 **한 줄**(말줄임) → 버튼. "좋아요" 4개가 진행 카드 포함 **스크롤 없이 한 화면**에 들어오는 밀도
- **완료·쉬어가기 행은 목록 맨 위로 정렬**(미완료 카드가 아래). 행 오른쪽 X 아이콘, **행 탭 → 확인창 없이 활성 카드로 복귀**(완료 실수 방지, history도 함께 되돌림)
- 완료된 루틴: 연녹(#EFF4EE) 행 + 딥그린 체크 원 + 취소선 + X(#6FB988)
- 활성 루틴 카드(흰): 버튼 [완료(노랑 #F7EBBE)] [지금은 어려워요(회색)] — 버튼은 전폭이 아닌 내용 폭(좌측 정렬)
- 쉬운버전 카드: 일반 카드와 같은 뼈대 + 이름 아래 배지 줄(슬림 배지 + 우측 "원래 버전으로 ›" 링크, 보통 버전으로 복귀·왕복 가능) + [완료] [오늘은 쉬어가기(코랄)]. 원래명 취소선은 컴팩트 개편 때 제거
  - 배지 문구 분기 (7/22): 시스템이 처음부터 쉬운 버전으로 뽑은 카드(onemove_easy_auto)는 "오늘은 쉬운 버전으로 준비했어요", 사용자가 '지금은 어려워요'로 바꾼 카드는 "더 쉬운 버전으로 바꿨어요". 자동 카드를 사용자가 되돌리면 이후엔 사용자 선택으로 취급
- **다른 루틴으로 바꾸기 (7/23)**: 활성 카드 칩 행 우측 **새로고침 아이콘** — '하기 싫은 루틴'을 같은 난이도의 다른 루틴으로 교체. **카드당 1회 제한**(교체로 들어온 카드엔 아이콘 없음 — 무한 리롤·회피 방지), ★ 매일 루틴 제외. 교체 후보는 pickSwapCandidate(routinePicker.js): 표시 난이도 유지, 오늘 세트·교체로 내보낸 루틴 제외, 영역 겹침·어제 루틴은 단계적 회피. 교체로 들어온 쉬운버전은 '준비했어요' 배지(자동 취급). 코치 모달 없이 조용히 교체
