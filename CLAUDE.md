...
- **사계절 앨범 (7/20 구현)**: 별도 저장 없이 기록에서 지난 계절 계산(getPastSeasons — 완료 1개 이상 계절만, 로그인 동기화로 자동 복원). 앨범 카드에 미니 정원 썸네일(가로 스크롤) → 탭하면 모달 열람(그 계절 완료 수·요소 수). 장면은 GardenScene 컴포넌트로 본 화면과 공용, SEASON_SETS에 계절 세트 등록(현재 여름만 — 가을 등 에셋 도착 시 추가). **?album=숫자**로 작년 같은 계절 샘플 미리보기(데모용). 실사용자 첫 앨범은 9/1(가을 시작)에 생김
