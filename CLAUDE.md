...
- 쉬운버전 카드: 별 배지 + 원래명 취소선 + 새 루틴명 + [완료] [오늘은 쉬어가기(코랄)] + 하단 "원래 버전으로 하기 ›" 작은 링크(보통 버전으로 복귀, 왕복 가능)
...
- **다른 루틴으로 바꾸기 (7/23)**: 활성 카드(일반·쉬운버전) 하단 작은 링크 — '하기 싫은 루틴'을 같은 난이도의 다른 루틴으로 교체. **카드당 1회 제한**(교체로 들어온 카드엔 링크 없음 — 무한 리롤·회피 방지), ★ 매일 루틴 제외. 교체 후보는 pickSwapCandidate(routinePicker.js): 표시 난이도 유지, 오늘 세트·교체로 내보낸 루틴 제외, 영역 겹침·어제 루틴은 단계적 회피. 교체로 들어온 쉬운버전은 '준비했어요' 배지(자동 취급). 코치 모달 없이 조용히 교체
