...
- **연출 3종 (7/27 멘토링 피드백)**: ① 헤드라인 타이핑 효과(95ms/자, 캐럿 블링크, 두 줄 높이 고정) ② 마우스 따라 메인 3색 블롭 배경 무빙(rAF 러프, 리렌더 없이 DOM 직접 변형 — blur 70~80px радиal 코랄/옐로/그린) ③ 코치 3종 꺽쇠 캐러셀(이름·tagline, key 교체 페이드). 전부 데스크톱 전용 + prefers-reduced-motion 시 정지(타이핑은 전문 즉시 표시)
- **About 모달**: 만든 사람(김서우)·한 줄 소개·기획 배경·취지·이메일(seowoo92@gmail.com) — 문안은 초안(추후 수정 예정), 점증 3점 마크
