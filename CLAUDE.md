...
- 별도 저장 없음 — onemove_history(게스트)/daily_entries(로그인)에서 계산. 요소 정의·계절 계산은 src/lib/garden.js
- 현재는 준비 화면(계절·완료 수·언락 요소 칩·다음 요소 진행 바) — **일러스트 도착 후 본 화면 구현** (배경+요소 11장 명세: docs/garden-asset-guide.md). 본 화면은 요소 상시 애니메이션(CSS, 구름 흐름·꽃 살랑·나비 비행 등) + prefers-reduced-motion 시 정지
...
- **정원 본 화면 구현** — 사용자 일러스트 11장 대기 (명세: docs/garden-asset-guide.md), 도착 시 요소 배치+상시 애니메이션+앨범 구현
