...
- **7/26 (확정 순서)**: ① 멘토링(18시) 대비 **기획안 초안(텍스트)** ② **사용자 안내서**(설정 탭 — ★ 매일 루틴·마음 날씨·쉬운 버전/원래 난이도·다른 루틴으로 바꾸기·완료 체크 해제·영역 7종·정원·앨범·카톡 알림/재발송) ③ 멘토링 후: **웹 푸시 알림**(푸시 전용 SW + VAPID 발송 서버, 3~5h — 실기기 검증 필요) + **실기기 최종 전수 점검** ④ 시간 되면 리포 정리
- **7/27~28**: 시연영상 촬영·편집(새 기능 동선 반영: 코치 한마디·루틴 교체·체크 해제·새 모달), PPT·노션 제작, 리포 정리(과거 커밋 작성자 정정·발표 문서 2건 히스토리 삭제 — 백업 후 force push, Solar 키 재발급·구키 폐기, README 정비), 코드 프리즈
- **7/29**: 오전 9~10시 제출 목표 (11시 마감), 최종 점검
- 예선 이후: 가을 에셋 세트(9/1 전), 닉네임 인라인 입력, 챗봇형 코치 대화(선택지 기반 1단계), 비 온 날 완료 시 무지개 배너 분기, 홈 완료 카드→행 FLIP 애니메이션
