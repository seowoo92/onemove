...
- 데스크톱 폰 프레임 (7/26 리디자인, 사용자 확정 2안): 앱 영역 390px×812px **고정**(창 높이를 늘려도 콘텐츠가 넘지 않음), **노치 없음**(상단 패딩 48→26px). 프레임 = 래퍼 div padding 8px + 딥그린 그라데이션(150deg #2F604B→#1C3F2F→#142E22) + radius 48/안쪽 40 + 그린 그림자·상단 inset 하이라이트. 페이지 배경은 화이트 유지
...
- 카드 아래 안내 두 줄(사용자 확정): "어떤 날이든 괜찮아요." + "마음 날씨에 맞춰, 오늘 할 수 있는 만큼만 추천할게요." — 무판단 신호는 날짜 옆에서 이곳으로 이동, 도움말 말풍선 2개는 7/24 제거(사용자 안내서가 대체). 날짜 부제는 날짜만
- **리셋 시점 (7/24)**: 오늘 기록 초기화는 '마음 날씨 다시 고르기'를 누를 때가 아니라 **새 날씨를 선택하는 시점**(App.handleStateSelect)에 수행 — 이 화면에서 뒤로 가면 아무것도 잃지 않음. confirm 경고는 진입 전(홈·설정) 유지
- 뒤로 버튼: 온보딩 진입 → S1(코치 유지) / 홈·설정의 '다시 고르기' 진입(stateCheckFrom='home') → **홈으로 복귀**(기록 그대로)

### S3 — 오늘의 루틴 홈 (Home.jsx)
- 헤더: "OO님, 오늘도 와줘서 고마워요" + 날짜·마음 날씨 부제
- 진행 카드(딥그린): "오늘 N / M개 완료" + "한 걸음씩"(옐로) + 진행 바(그라데이션 #D9F2EE→#F3D978)
- **카드 레이아웃 (7/23 최종 확정)**: 칩 행(**좌측 ★·새로고침 아이콘 / 우측 난이도·영역 칩** — 컬러 균형) → 루틴명 **한 줄**(말줄임) → 버튼 행. "좋아요" 4개가 진행 카드 포함 **스크롤 없이 한 화면**
- 버튼 (사용자 확정): **완료 = 노랑(#F7EBBE) 필**(우측), **보조(지금은 어려워요·오늘은 쉬어가기) = 회색 테두리(#E2DFD6) 아웃라인 필** — 우측 정렬, 슬림(padding 7px/6px)
- **완료·쉬어가기 행은 목록 맨 위로 정렬**(미완료 카드가 아래). **왼쪽 체크 원을 누르면 체크 해제 → 카드 복원**(완료 실수 방지, history도 되돌림. X 아이콘은 삭제로 오인되어 제거) — 사용자 안내서에 기재 예정
- 완료된 루틴: 연녹(#EFF4EE) 행 + 딥그린 체크 원(탭=해제) + 취소선
- 쉬운버전 카드: 일반 카드와 **완전히 같은 뼈대**. 칩 행의 난이도 칩 왼편에 "**원래 버전으로 ⇄**"(위아래 맞바꿈 화살표) 링크 — 보통 버전으로 복귀·왕복 가능. [오늘은 쉬어가기(코랄 텍스트 아웃라인)] [완료]
  - **배지 (7/23 확정)**: 시스템이 자동으로 쉬운 버전을 뽑은 카드는 **배지 없음**(일반 카드와 동일 밀도). 사용자가 '지금은 어려워요'로 바꾼 카드에만 "더 쉬운 버전으로 바꿨어요" 배지(행동 피드백). onemove_easy_auto로 구분
- **다른 루틴으로 바꾸기 (7/23)**: 활성 카드 칩 행 좌측 **새로고침 아이콘** — '하기 싫은 루틴'을 같은 난이도의 다른 루틴으로 교체. **카드당 1회 제한**(교체로 들어온 카드엔 아이콘 없음 — 무한 리롤·회피 방지), ★ 매일 루틴 제외. 교체 후보는 pickSwapCandidate(routinePicker.js): 표시 난이도 유지, 오늘 세트·교체로 내보낸 루틴 제외, 영역 겹침·어제 루틴은 단계적 회피. 코치 모달 없이 조용히 교체
- **하단 보조 액션**: "마음 날씨 다시 고르기" + (알림 동의 사용자만) "**오늘 루틴 카톡으로 받기**" — 루틴 교체·난이도 조절로 구성이 바뀐 뒤 수동 재발송(resendRoutineCard, 하루 1회 제한 없음·사용자 탭으로만, 성공/실패 alert). 2단 그리드 가운데 정렬, **완료 축하 상태(allResolved)에선 숨김**
- **코치 한마디 플로팅 (7/24)**: 좌하단 코치 얼굴 원(48px, coach-face-*.png) + 연그린 반투명(0.86+blur) 말풍선 — 코치 존재감 강화(피드백 반영). 진입 시 한마디 → 5초 후 자동 접힘 → 얼굴 탭마다 직전과 다른 문구 랜덤. 문구는 src/lib/coachTalk.js 성격별 20개(사용자 확정, 전부 말풍선 1줄, 입말·권유 금지 톤). 챗봇형 대화는 안전 설계 필요로 예선 이후 로드맵
- 영역칩 색: 몸 깨우기·자기돌봄·에너지 → 초록(#D5EFD8), 공간·바깥 → 파랑(#E4F0F6), 연결·성취 → 코랄(#FBE3DA)
- 영역칩 표시명 (7/23 사용자 확정): 단독 노출 시 뜻이 통하도록 화면 라벨만 보완 — 몸 깨우기→'몸 움직이기', 바깥→'바깥 활동', 공간→'공간 정리', 연결→'마음 연결', 에너지·자기돌봄·성취는 그대로 (Home.jsx AREA_LABEL, routines.js의 area 값은 그대로)
...
- **하루 마무리(AI 회고)**: 완료 축하 카드 안에 표시 — 완료 루틴들의 영역을 묶어 하루를 종합하는 2문장(generateDailyReview) + 배지 "AI 생성"/"예비 문구"(7/26 문구 확정). 하루 1회 생성 후 onemove_review에 저장·재사용(자정 리셋, 마음 날씨 재선택 시 함께 제거). 전부 쉬어간 날은 수용·안심 톤 분기
- 하단 "마음 날씨 다시 고르기" 작은 버튼

### S4 — 코치 메시지 모달 (CoachModal.jsx, 바텀시트)
- **탭바 위에 얹힌 시트 (7/24 확정)**: 시트 하단이 탭바 상단에 **여백 0으로 밀착**(paddingBottom = 탭바 높이 56.3px+안전영역), 상단만 28px 라운드. 탭바는 보이지만 탭하면 딤 클릭으로 모달만 닫힘(탭 이동 연결 안 함 — 사용자 결정). 슬라이드업+딤 페이드 진입, 드래그 핸들 없음(드래그 동작이 없어 제거)
- **스케치 디자인**: 메시지 **말풍선**(꼬리 포함, 폰트 16px) + **선택한 코치 상반신 캐릭터**(public/images의 coach PNG, 시트 하단 경계 아래로 블리드해 잘리며 올라옴, overflow:hidden으로 클립) + 코치 이름 + "방금 도착한 한마디" + [계속하기]
...
- 출처 배지는 7/24 제거(필수 아님). 하루 마무리 카드에만 "AI 생성"/"예비 문구" 배지 유지(7/26 확정)
...
- **데모 파라미터** (발표 시연·개발 확인용, 실제 저장 영향 없음): `?garden=숫자` 정원 완료 수 가상 지정 / `?record=demo` 기록 탭 3주치 샘플 기록(고정 패턴, src/lib/demoRecord.js) / `?crash=1` 에러 안내화면 확인
...
- **사용법 안내 (7/26)**: 설정 헤더 우측 "사용법 안내 ›" 연녹 칩(B안 — 한 화면 원칙 유지)으로 GuideScreen 진입. 섹션 3개(오늘의 루틴/기록과 정원/알림)를 **흰 그룹 카드 + 연녹(#EFF4EE) 헤더 띠**로 묶고 항목 9개는 카드 안 아코디언 행(구분선, grid-rows 펼침) — 섹션·항목 폰트 14px 통일(사용자 확정 3안). 항목: 마음 날씨·쉬운 버전 ⇄·루틴 교체·매일 루틴 ★·쉬어가기·체크 해제·영역 7종(실제 칩)·정원과 앨범·카톡 알림/재발송. 하단 패딩 96px(고정 탭바 가림 방지, 기록 탭과 동일)

### 데스크톱 좌측 소개 패널 (AppLayout 데스크톱 모드)
- 클레이 3점 브랜드 마크 + 헤드라인("오늘 할 수 있는 / 만큼만.") + 핵심 3가지 + **코치 캐러셀** + 구분선 + **QR 코드**(public/images/qr-onemove.svg) + 슬로건 + **About 메뉴·저작권**(폰 프레임 하단 라인 정렬, 패널 높이 828px 컬럼)
- **연출 5종 (7/27 멘토링 + 7/28 피드백)**: ① 헤드라인 타이핑 효과(캐럿 블링크) ② 메인 3색 대형 원(동일 520px, blur 60) — **자율 부유**(blob-drift 래퍼 transform, 26/30/34s alternate) + 마우스 패럴랙스(안쪽 .blob-inner에 rAF 러프 — 래퍼/안쪽 분리로 transform 충돌 방지) ③ 코치 3종 꺽쇠 캐러셀 ④ **진옐로(#E8C24E) 커서 링** — lerp 팔로우, button/a 위 52px 확대, `pointer: fine` 환경 전용(헤드리스 검증 시 matchMedia 스텁 필요) ⑤ About 버튼 둥둥 부유(about-float, 래퍼 적용). 전부 데스크톱 전용 + prefers-reduced-motion 시 정지
- **About 모달 (7/28 책갈피 5탭 확장, A안 사용자 확정)**: 780×560 2단 — 좌측 책갈피(01 만든 사람/02 기획 의도/03 스토리/04 기술 스택/05 앞으로의 계획, 활성=흰 카드+코랄 바) + 우측 내용(탭 페이드, 내용만 스크롤). 트리거는 'About 오늘만큼' 아웃라인 필 버튼(부유). 스토리 탭=확정 스토리텔링 서사, 모달 내 저작권 없음(패널 하단 유지)
...
| `onemove_easy_auto` | 시스템이 처음부터 쉬운 버전으로 준비한 루틴 ID (배지 문구 분기용, daily_entries.easy_auto_ids로 동기화) | 매일 |
| `onemove_swap_used` | 교체로 들어온 루틴 ID — 재교체 불가 (daily_entries.swap_used_ids로 동기화) | 매일 |
| `onemove_swapped_out` | 교체로 내보낸 루틴 ID — 오늘 재추천 제외 (daily_entries.swapped_out_ids로 동기화) | 매일 |
...
- 매핑 레이어: src/lib/weather.js
  - 좋아요 → 맑음, 설명: "오늘은 마음도 화창해요", 색: #F3D978
  - 보통이에요 → 구름, 설명: "구름 조금, 그런 날도 있죠", 색: #9FD2B0
...
## 에셋 (public/images/)
- 코치 캐릭터 3종: coach-cheerful|calm|warm.png (상반신 클레이 일러스트, 좌우 투명 여백 ~20%, 7/24 경계 디프린지·페더링 처리)
- 코치 얼굴 크롭 3종: coach-face-cheerful|calm|warm.png (96px, 코치 한마디 플로팅 아바타용 — 상반신 원본에서 x30~70%·y6~46% 크롭)
...
## 현재 상태 (2026-07-28 새벽 — 사용자 취침, 다음 세션은 PPT 목차 논의부터)
- **예선 제출 7/29(화) 오전 11시** (9~10시 제출 목표). 개발 완료, 자료 제작 단계
- **다음 세션 첫 작업**: **PPT 목차·세부내용을 사용자와 논의**(초안: docs/ppt-outline.md — 14장 안 + 논의거리 5개. 바로 제작 금지, 논의 후 제작이 사용자 지시). 이후 기획서 PDF에 경쟁 분석 섹션 추가 → 리포 정리 → 시연영상(사용자 실기기 촬영). **노션은 생략(사용자 결정)**, 자료 순서: PPT → 리포 정리 → 영상
- 7/27 저녁~새벽: 지인 테스트 버그 3건 수정(설치형 PWA 노치 가림 — safe-area-inset-top 부재 / 하루 마무리 예비 문구 하루 고정 → 재시도·배지 숨김 / 진중 문구 교체) + **경쟁 분석 확정본 docs/competitor-analysis.md**(국내 4종 마이루틴·빛나길·무디·디스턴싱 — 16축 비교표·어필 5·예상 질문 3·스토리텔링 결론 3버전"직접 써봤다" 기조) — 상세 devlog 2026-07-27.md 6~7절
- 7/26: 기획안(md+PDF 13쪽) · 사용자 안내서 · 웹 푸시 완성(실기기 검증) · alert→토스트 · 카카오 카드 텍스트 확정 — 상세 docs/devlog/2026-07-26.md
- 7/27: 온보딩 가이드 4장(실화면 미니 프레임·브라우저 감지 설치 안내) · 코치 문구 6건 검수 교체 · 멘토링 2차 피드백 전면 반영(도움말 풍선·가독성·About·저작권·데스크톱 연출 3종) · 좌측 패널 리디자인(캐릭터 180px 2단·타이핑·블롭·캐러셀) · 텍스트 선택 차단 — 상세 docs/devlog/2026-07-27.md
- **세션 밖 백업**: VAPID 비밀키 → 리포 루트 `vapid-keys.local`(gitignore, 분실 시 기존 구독 전멸) · 기획 보고서 소스(HTML·PDF·컷) → `docs/ppt-build/기획보고서/`(gitignore, PPT 재료)
- 미사용 잔재: public/images/kakao-banner-*.png (피드 실험용 — 리포 정리 때 제거 가능)
...
## 지난 상태 (2026-07-23 새벽)
- **루틴 카드 대개편**: 칩 행(좌 아이콘/우 칩)→이름 한 줄→노랑 필+아웃라인 버튼, 4개 한 화면(760px 검증), 완료 행 상단 정렬+체크 해제 복원, 헤더 ScreenHeader 통일
- **자율성 기능 3종**: 다른 루틴으로 바꾸기(카드당 1회)·원래 난이도로 ⇄ 왕복·쉬운버전 배지 정직화(자동은 무배지) — daily_entries에 easy_auto_ids·swap_used_ids·swapped_out_ids 열 추가 완료
- **카톡 루틴 카드 수동 재발송** 버튼(홈 하단, 알림 사용자) — 실기기 수신 확인 대기
- 영역 칩 표시명 확정(몸 움직이기·바깥 활동·공간 정리·마음 연결) · dev 환경 복구(.env.local anon 키, manifest 경로)
- 시연 동선 웹 리허설 통과(이슈 0), 상세: docs/devlog/2026-07-23.md

## 지난 상태 (2026-07-22)
- **스플래시 스크린** (index.html 인라인 — 클레이 로고 1.5배 확정) · **에러 화면 브랜딩** ("잠깐 소나기를 만났어요", ?crash=1)
- 예선 일정 확정: 7/28 11시 자료 4종(PPT·노션·깃허브·영상), 24일 코드 프리즈·리포 정리, 상세 남은 일 참조
...
## 남은 일 (예선 제출 **7/29(화) 오전 11시** — 예선은 서면 자료 평가·발표 없음, 발표는 본선)
- **순서 (사용자 확정)**: ① PPT(목차 논의 → 제작, 서면 심사용 "읽는 장표". 초안 docs/ppt-outline.md) ② 리포 정리(과거 커밋 작성자 정정·발표 문서 2건 히스토리 삭제 — 백업 후 force push, Solar 키 재발급·구키 폐기, README 정비, kakao-banner 잔재 제거) ③ 시연영상(실기기 촬영 — 온보딩·코치 한마디·루틴 교체·체크 해제·정원·푸시 알림 동선). 노션 생략(사용자 결정 — 요강 감점 여부 확인 권장). 기획서 PDF에 경쟁 분석 섹션 추가(소스 docs/ppt-build/기획보고서/), 코드 프리즈
- **멘토링 2차(7/26) 자료 반영 항목**: 교육과정에서 배운 것→서비스 적용→기획 이유 스토리 / "주 사용은 모바일, 데스크톱은 목업 프레임으로 배려" 어필 / **트러블슈팅 서사**(7/6 백엔드 정지 흰 화면→3초 제한+ErrorBoundary, KOE205→비즈앱 전환, 카카오 피드 한계 검증→텍스트 확정, iOS 웹푸시 제약→설치 안내 설계, 모달 z-index 회귀 등) / 경쟁 앱 비교 섹션
- **7/29**: 오전 9~10시 제출 목표 (11시 마감), 최종 점검
- About 문안은 초안 상태 — 사용자가 추후 수정 예정
- 예선 이후: 가을 에셋 세트(9/1 전), 닉네임 인라인 입력, 챗봇형 코치 대화(선택지 기반 1단계), 비 온 날 완료 시 무지개 배너 분기, 홈 완료 카드→행 FLIP 애니메이션, 카카오 루틴 카드 동적 이미지 생성(카드 한 통에 목록까지)
