...
- 영역칩 표시명 (7/23 사용자 확정): 단독 노출 시 뜻이 통하도록 화면 라벨만 보완 — 몸 깨우기→'몸 움직이기', 에너지→'에너지 유지', 바깥→'바깥 활동', 공간→'공간 정리', 연결→'마음 연결', 자기돌봄·성취는 그대로 (Home.jsx AREA_LABEL, routines.js의 area 값은 그대로)
...
- **7/22**: ~~스플래시 스크린~~·~~에러 안내화면~~ 완료 → (여유) 완료 무지개·랜덤 힐링 문구·데모 동선 점검
- **7/23**: 추가 개발·디자인·버그 수정, 실기기 전수 점검·시연 동선 리허설
