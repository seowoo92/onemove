...
- 대회 출품작: '2026 리부트 AI 활용대회' (정식 명칭 — 문서 표기 시 이 명칭 사용)
...
## 현재 상태 (2026-08-02 — 컨텍스트 클리어 직전)
- 개발 완료·실서비스 운영 중. 7/27 지인 테스트 버그 3건 수정 배포(노치 safe-area·하루 마무리 예비 문구 재시도·배지 정책·코치 문구)
- 데스크톱 연출 5종 + About 책갈피 5탭 모달 완성(7/28) — 상세는 '디자인 시스템'·'데스크톱 좌측 소개 패널' 절
- **다음 세션: 시연영상 논의부터 시작** (마감 8/2(일) 18시, 본인 목소리 필수·기술/AI 활용이 영상에 보여야 함)
- **제출자료 관련 모든 메모·산출물은 docs/ppt-build/NOTES.md 참조 (저장소 미포함)** — 개발일지·CLAUDE.md에는 개발 내용만 기록하는 원칙
...
## 남은 일
- 시연영상 준비 지원 (다음 세션 — 상세 요건은 docs/ppt-build/NOTES.md)
- 리포 정리: README 정비, 미사용 에셋 제거(public/images/kakao-banner-*), 과거 커밋 정리 여부는 NOTES.md 참조
- 예선 이후: 가을 에셋 세트(9/1 전), 닉네임 인라인 입력, 챗봇형 코치 대화(선택지 기반 1단계), 비 온 날 무지개 배너, 홈 완료 카드 FLIP 애니메이션, 카카오 루틴 카드 동적 이미지
