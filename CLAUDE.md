...
## 현재 상태 (2026-07-27 오후 — 컨텍스트 클리어 직전)
- **예선 제출 7/29(화) 오전 11시** (9~10시 제출 목표). 개발은 사실상 완료 — 멘토링 2차 피드백 개발 항목 전부 반영됨
- **다음 세션 첫 작업 (사용자 지시 순서)**: ① **버그 수정** — 사용자가 발견한 버그, 내용은 사용자에게 물어볼 것 ② **경쟁 앱 비교 리서치**(실제 앱 명시 비교표 → 기획서·PPT) ③ 자료 4종(영상·PPT·노션·리포 정리)
...
## 남은 일 (예선 제출 **7/29(화) 오전 11시** — 자료 4종: PPT·노션·깃허브·시연영상, 예선은 자료 평가·발표는 본선)
- **즉시**: ① 버그 수정(사용자 발견 — 내용 문의) ② 경쟁 앱 비교 리서치(마이루틴·루티너리·챌린저스 등 실제 앱 명시, 차별점 비교표 → 기획서·PPT 섹션)
- **자료 4종 (7/27 저녁~28)**: 시연영상(실기기 촬영 — 온보딩·코치 한마디·루틴 교체·체크 해제·정원·푸시 알림 동선), PPT·노션 제작, 리포 정리(과거 커밋 작성자 정정·발표 문서 2건 히스토리 삭제 — 백업 후 force push, Solar 키 재발급·구키 폐기, README 정비), 코드 프리즈
